...
function getAllGameObjectsByGameInstanceKey($gameKey,$gameInstanceKey,&$db,&$arrayResponse)
{
	$arrayResponse = array();
	$arrayDetails = array();
	$arrayDetails["action"] = "getAllGameObjectsByGameInstanceKey";
	...
	try{	
		...
		$sql = "SELECT object.objectKey FROM `faars-rocketfuel`.object WHERE object.gameInstance_ID=:gameInstance_ID";
		$stmt = $db->prepare($sql);
		...
		{
			$arrayResponse = array();
			$arrayResponse["status"] = 12;
			$arrayResponse["message"] = "No game object entries found in database with given game instance key";
